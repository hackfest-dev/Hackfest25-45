...
const CONFIG = {
    ...
    voices: [
        ...
        { id: 'pNInz6obpgDQGcFmaJgB', name: 'Adam', gender: 'male', accent: 'British' },
        { id: 'yoZ06aMxZJJ28mfd3POQ', name: 'Sam', gender: 'male', accent: 'British' }
    ],
    typography: {
        largeTitle: '28px',
        title1: '22px',
...
const appState = {
    ...
    selectedVoice: '21m00Tcm4TlvDq8ikWAM', // Default to Rachel
    isSpeaking: false
};
...
function setupUI() {
    ...
    // Settings Panel with Apple-style card
    const settingsCard = document.createElement('div');
    settingsCard.style.width = '100%';
    settingsCard.style.backgroundColor = CONFIG.colors.cardBackground;
    settingsCard.style.borderRadius = CONFIG.cornerRadius.medium;
    settingsCard.style.padding = CONFIG.spacing.medium;
    settingsCard.style.boxShadow = '0 1px 3px rgba(0,0,0,0.05)';
    ...
    settingsCard.appendChild(toneMenuContainer);

    // Voice Selection Menu
    const voiceMenuContainer = document.createElement('div');
    voiceMenuContainer.style.width = '100%';
    voiceMenuContainer.style.display = 'flex';
    voiceMenuContainer.style.flexDirection = 'column';
    voiceMenuContainer.style.gap = CONFIG.spacing.small;
    voiceMenuContainer.style.marginTop = CONFIG.spacing.medium;
    
    const voiceLabel = document.createElement('div');
    voiceLabel.textContent = 'SELECT VOICE';
    voiceLabel.style.fontWeight = '600';
    voiceLabel.style.fontSize = CONFIG.typography.footnote;
    voiceLabel.style.color = CONFIG.colors.secondaryText;
    voiceLabel.style.letterSpacing = '0.5px';
    voiceLabel.style.textTransform = 'uppercase';
    voiceMenuContainer.appendChild(voiceLabel);
    
    const voiceSelect = document.createElement('select');
    voiceSelect.id = 'voice-select';
    voiceSelect.style.width = '100%';
    voiceSelect.style.padding = '10px 12px';
    voiceSelect.style.borderRadius = CONFIG.cornerRadius.medium;
    voiceSelect.style.border = `1px solid ${CONFIG.colors.separator}`;
    voiceSelect.style.backgroundColor = CONFIG.colors.cardBackground;
    voiceSelect.style.color = CONFIG.colors.text;
    voiceSelect.style.fontSize = CONFIG.typography.subhead;
    voiceSelect.style.fontFamily = 'inherit';
    voiceSelect.style.appearance = 'none';
    voiceSelect.style.webkitAppearance = 'none';
    voiceSelect.style.backgroundImage = 'url("data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%231C1C1E%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E")';
    voiceSelect.style.backgroundRepeat = 'no-repeat, repeat';
    voiceSelect.style.backgroundPosition = 'right 12px top 50%, 0 0';
    voiceSelect.style.backgroundSize = '12px auto, 100%';
    ...
    CONFIG.voices.forEach(voice => {
        ...
        if (voice.id === appState.selectedVoice) {
    ...
    voiceMenuContainer.appendChild(voiceSelect);
    settingsCard.appendChild(voiceMenuContainer);
...
function handlePrediction(prediction) {
    ...
    if (prediction === "no gesture") {
        ...
        // Removed the speak() call here to only speak complete sentences
    }
}
...
async function speak(text) {
    ...
    try {
        const response = await fetch(`${CONFIG.elevenLabs.apiEndpoint}/${appState.selectedVoice}`, {
            method: 'POST',
            headers: {
